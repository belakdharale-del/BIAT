...
<!-- END BIAT_UNIFIED_FUTURE_STYLE_SIDEBAR -->
'''
...
for file in pages:
    ...
    p.with_name("code_backup_before_unified_future_sidebar.html").write_text(html, encoding="utf-8")

    # Remove previous injected sidebar/style blocks
    for marker in [
    ...
    # If no aside existed, inject after body
    if "BIAT_UNIFIED_FUTURE_STYLE_SIDEBAR" not in html:
        html = re.sub(r'<body([^>]*)>', r'<body\1>' + sidebar, html, count=1, flags=re.IGNORECASE)

    p.write_text(html, encoding="utf-8")
    print("Unified sidebar installed:", file)

print("DONE: all pages now use Future Prediction sidebar style.")
